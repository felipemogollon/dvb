...
int main(int argc, char **argv)
{
	...
	AM_FileEcho("/sys/class/stb/dvr0_mode", "ts");
	if(!freq)
	{
		freq = 802000;
	}
...
/*
	...
	for (i=0; i< DVR_DEV_COUNT; i++)
	{
		...
		AM_DMX_SetSource(i, AM_DMX_SRC_TS1);
		AM_DEBUG(1, "Openning DVR%d...", i);
		memset(&dpara, 0, sizeof(dpara));
		AM_TRY(AM_DVR_Open(i, &dpara));
		if(i ==0)
			AM_DVR_SetSource(i,AM_DVR_SRC_ASYNC_FIFO0);
		else
			AM_DVR_SetSource(i,AM_DVR_SRC_ASYNC_FIFO1);
