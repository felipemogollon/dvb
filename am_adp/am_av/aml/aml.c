...
/**\brief 设置数据注入参赛*/
static AM_ErrorCode_t aml_start_inject(AM_AV_Device_t *dev,AV_InjectData_t *inj, AM_AV_InjectPara_t *para,AM_Bool_t create_thread)
{
	int vfd=-1, afd=-1;
	...
	if (para->vid_id >= 0x1fff || para->aud_id >= 0x1fff)
	{
	...
	if(vfd!=-1)
	{
		...
		ioctl(inj->cntl_fd, AMSTREAM_IOC_VPAUSE, 1);
	}
	...
	AM_TIME_GetClock(&dev->inject_player.av_start_time);
	dev->canInjectData = AM_TRUE;
	/*创建AV监控线程*/
	if (create_thread)
	{
		dev->inject_player.av_thread_running = AM_TRUE;
		if(pthread_create(&dev->inject_player.av_mon_thread, NULL, aml_av_monitor_thread, (void*)dev))
		{
			AM_DEBUG(1, "create the av buf monitor thread failed");
			dev->inject_player.av_thread_running = AM_FALSE;
		}
	}
...
/**\brief 释放数据注入相关数据*/
static void aml_destroy_inject_data(AM_AV_Device_t *dev,AM_Bool_t destroy_thread)
{
	AV_InjectData_t *inj;

	if (destroy_thread && dev->inject_player.av_thread_running) 
	{
		dev->inject_player.av_thread_running = AM_FALSE;
		pthread_cond_broadcast(&gAVMonCond);
		pthread_join(dev->inject_player.av_mon_thread, NULL);
	}
	dev->canInjectData = AM_FALSE;
...
static AM_ErrorCode_t aml_start_ts_mode(AM_AV_Device_t *dev, AV_TSPlayPara_t *tp, AM_Bool_t create_thread)
{
	...
	if(ts->fd == -1)
	{
		AM_Bool_t have_video = AM_FALSE, have_audio = AM_FALSE;
		...
		if((tp->apid > 0) && (tp->apid<0x1fff))
		{
			have_audio = AM_TRUE;
		}

		AM_DEBUG(1, "aml_start_ts_mode used MPTSONLYAUDIOVIDEO %d %d %d %d", tp->vpid, have_video, tp->apid, have_audio);
...
static void* aml_av_monitor_thread(void *arg)
{
	...
	AM_Bool_t has_audio = ((dev->mode&AV_PLAY_TS)?dev->ts_player.play_para.apid:dev->inject_player.play_para.aud_id) < 0x1fff;
	AM_Bool_t has_video = ((dev->mode&AV_PLAY_TS)?dev->ts_player.play_para.vpid:dev->inject_player.play_para.vid_id) < 0x1fff;
	...
	while((dev->mode&AV_PLAY_TS)?dev->ts_player.av_thread_running:dev->inject_player.av_thread_running){
		fd = get_amstream(dev);
		cntl_fd = (dev->mode&AV_PLAY_TS)?((AV_TSData_t*)dev->ts_player.drv_data)->vid_fd:((AV_InjectData_t *)dev->inject_player.drv_data)->cntl_fd;
		if (!adec_start)
		...
		if (! ((dev->mode&AV_PLAY_TS)?dev->ts_player.av_thread_running:dev->inject_player.av_thread_running))
			break;
		...
		if(ioctl(fd, AMSTREAM_IOC_VB_STATUS, (unsigned long)&vstatus) != -1){
		...
		if(has_audio && !adec_start){
			...
			if(adec_start){
				...
				memset(&info, 0, sizeof(info));
				info.valid  = 1;
				ioctl(fd, AMSTREAM_IOC_AUDIO_INFO, (unsigned long)&info);

				adec_start_decode(fd, (dev->mode&AV_PLAY_TS)?dev->ts_player.play_para.afmt:dev->inject_player.play_para.aud_fmt);
		...
		if(!av_paused){
			...
			if(av_paused){
				...
				if(has_video){
					ioctl(cntl_fd, AMSTREAM_IOC_VPAUSE, 1);
				}
		...
		if(av_paused){
			...
			if(!av_paused){
				...
				if(has_video){
					ioctl(cntl_fd, AMSTREAM_IOC_VPAUSE, 0);
				}
				apts_stop_time = 0;
				vpts_stop_time = 0;
				resample_type = 0;
		...
		if(!no_audio_data && (dmx_apts_stop_dur > NO_DATA_CHECK_TIME) && (apts_stop_dur > NO_DATA_CHECK_TIME)){
			AM_Bool_t sf[2];
		...
		if(!no_video_data && (dmx_vpts_stop_dur > NO_DATA_CHECK_TIME) && (vpts_stop_dur > NO_DATA_CHECK_TIME)){
			AM_Bool_t sf[2];
			AM_DMX_GetScrambleStatus(0, sf);
			if(sf[1]){
		...
		if(need_replay){
			...
			{
				aml_destroy_inject_data(dev,AM_FALSE);
				aml_open_mode(dev, AV_INJECT);
				aml_start_inject(dev,dev->inject_player.drv_data,&dev->inject_player.play_para,AM_FALSE);
			}
			adec_start = AM_FALSE;
			av_paused  = AM_TRUE;
			resample_type = 0;
...
static AM_ErrorCode_t aml_start_mode(AM_AV_Device_t *dev, AV_PlayMode_t mode, void *para)
{
	...
	switch(mode)
	{
		...
		case AV_INJECT:
			...
			inj = dev->inject_player.drv_data;
			dev->inject_player.play_para = *inj_p;
			if(aml_start_inject(dev,inj, inj_p,AM_TRUE)!=AM_SUCCESS)
				return AM_AV_ERR_SYS;
		break;
		case AV_TIMESHIFT:
...
static AM_ErrorCode_t aml_close_mode(AM_AV_Device_t *dev, AV_PlayMode_t mode)
{
	...
	switch(mode)
	{
		...
		break;
		case AV_INJECT:
			aml_destroy_inject_data(dev,AM_TRUE);
		break;
		case AV_TIMESHIFT:
			tshift = dev->timeshift_player.drv_data;
...
static AM_ErrorCode_t aml_inject(AM_AV_Device_t *dev, AM_AV_InjectType_t type, uint8_t *data, int *size, int timeout)
{
	...
	if(!dev->canInjectData)
	{
			AM_DEBUG(1, "device not be opened,not inject data!\n");
